...
- [Simple Steam hour farmer](#simple-steam-hour-farmer)
...
```sh
npm init -y
npm install steam-hour-farmer
```

### Configuration

1. Find your Steam game IDs. In each game, go to Properties -> Updates -> Copy the App ID.  
Alternatively you can simply check the game link in the store, it should look something like this: `https://steamcommunity.com/app/427520`  
In this case `427520` is the game ID.

2. In the same directory, make a file named `.env` with the content:
    ...
    >
    > You can also omit `ACCOUNT_NAME` and `PASSWORD` to log-in using a **QR code** instead.

    This example will start playing Team Fortress 2 and Garry's Mod.
    ...
    Do not write this value if you don't want the user's presence to change.

    All of this configuration can be passed via environment variables too - they don't need to be in this `.env` file.

3. Run the program in the same directory:
...
- Task Scheduler on Windows systems
